...
def extract_abResult(tryptic, main_protein, path, sample_name):
    ...
    for enzyme_num in range(1, 6):
        ...
        #Get the sum of intensity for each peptide 
        peptide_intensity = {}
        for index, row in peptide_feature.iterrows():        
            peptide = row['Peptide']
            if peptide in peptide_intensity.keys():
                peptide_intensity[peptide] += row['Intensity']
            else:
                peptide_intensity[peptide] = row['Intensity']
                
        #print("peptide num: " + str(len(peptide_intensity)))
        ...
        with open(out_csv_name, 'w') as f:
            f.write("Peptide,Intensity\n")
            
            for key, intensity in peptide_intensity.items():
